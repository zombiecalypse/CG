...
public class ObjReader {
	 ...
	 * @throws IOException
	 */
	public static VertexData read(String fileName, float scale)
			throws IOException {
		...
		for (int i = 0; i < nFaces; i++) {
			...
			for (int j = 0; j < 3; j++) {
				...
				verticesFinal[vertexNr * 3] = vertices
						.get(faces.get(i)[j][0] - 1)[0];
				verticesFinal[vertexNr * 3 + 1] = vertices
						.get(faces.get(i)[j][0] - 1)[1];
				verticesFinal[vertexNr * 3 + 2] = vertices
						.get(faces.get(i)[j][0] - 1)[2];

				verticesFinal[vertexNr * 3] = scale
						* (verticesFinal[vertexNr * 3] + xTrans);
				...
				if (hasNormalIndices) {
					normalsFinal[vertexNr * 3] = normals
							.get(faces.get(i)[j][2] - 1)[0];
					normalsFinal[vertexNr * 3 + 1] = normals
							.get(faces.get(i)[j][2] - 1)[1];
					normalsFinal[vertexNr * 3 + 2] = normals
							.get(faces.get(i)[j][2] - 1)[2];
				...
				if (hasTexCoordIndices) {
					texCoordsFinal[vertexNr * 2] = texCoords
							.get(faces.get(i)[j][1] - 1)[0];
					texCoordsFinal[vertexNr * 2 + 1] = texCoords.get(faces
							.get(i)[j][1] - 1)[1];
		...
		if (texCoords.size() > 0)
			vertexData.addElement(texCoordsFinal, VertexData.Semantic.TEXCOORD,
					2);
